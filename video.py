# -*- coding: utf-8 -*-
"""Video 2.02 beta hudba formát iphone.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1ekXkUn6h_-R8RK3awD60f2ymC3oLwBt4
...
"""
OPRAVA SYNTAXE PRO GITHUB ACTIONS
Tento docstring pohltí případný chybně exportovaný text z markdown buněk.
"""
# Instalace a importy
...
if IS_COLAB:
    # Vyčištění předchozích souborů
    old_files = glob.glob("*.png") + glob.glob("*.jpg") + glob.glob("*.jpeg") + glob.glob("*.webp")
    for f in old_files:
        try: os.remove(f)
        except: pass

    print("🧹 Systém připraven. Vyberte fotografie.")

    try:
        # Použití standardního nahrávání, ale s vynuceným sekvenčním zápisem pro stabilitu na iOS
        ...
        if uploaded:
            for name, data in uploaded.items():
                ...
                with open(path, 'wb') as f:
                    f.write(data)
                # Okamžité uvolnění paměti z dictionary pro iOS stabilitu
                del uploaded[name]

            final_count = len(os.listdir(INPUT_DIR))
            if final_count > 0:
                print(f"\n✅ Úspěšně nahráno {final_count} souborů.")
            else:
                print("\n⚠️ Složka je prázdná, nahrávání selhalo na úrovni přenosu.")
        else:
            print("⚠️ Nahrávání bylo zrušeno.")

    except Exception as e:
        # Technická kontrola obsahu složky při chybě rozhraní
        existing_files = os.listdir(INPUT_DIR)
        if len(existing_files) > 0:
            print(f"\nℹ️ Nahrávání bylo dokončeno i přes chybu rozhraní ({len(existing_files)} souborů nalezeno).")
        else:
            print(f"\n❌ Chyba při přenosu: {e}")
